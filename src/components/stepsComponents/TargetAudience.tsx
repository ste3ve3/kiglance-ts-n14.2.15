...
import { updateTargetAudience } from "@/redux/features/UserExperienceSlice";
...
const TargetAudience = () => {
  ...
  const { targetAudience } = useAppSelector((state) => state.userExperience);
      ...
      <div className="w-2/3 h-[80%] flex flex-col items-center overflow-y-auto">
        ...
        <div className="grid grid-cols-1 gap-2 w-4/5 mt-3">
          {audiences.map((audience) => (
            <ValueBox
              key={audience}
              isSelected={targetAudience === audience}
              text={audience}
              handleClick={() => {
                dispatch(updateTargetAudience(audience));
              }}
            />
          ))}
        </div>
